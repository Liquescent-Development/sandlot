...
import { loadPolicyFiles } from "./config.js";
import { renderDiagnosticSnapshot, redactDiagnosticText } from "./diagnostics.js";
import { buildChildEnvironment, sandlotMktempShimDirectory } from "./environment.js";
import { assertProtectedOwnership, evaluateToolCall, protectedToolSourcePaths } from "./guard.js";
import { FileWorkerClient, resolveFileWorkerPaths, } from "./helpers/file-worker.js";
import { SearchWorkerClient, resolveSearchWorkerPaths, } from "./helpers/search-worker.js";
import { composePolicy, toSandboxRuntimeConfig, validatePolicyForPlatform, } from "./policy.js";
...
import { pinnedPiImageProcessor, resolvePinnedPiImagePaths, } from "./tools/files.js";
import { registerProtectedTools } from "./tools/index.js";
import { resolveExtensionTrustPaths, } from "./trust.js";
export { resolveExtensionTrustPaths } from "./trust.js";
export function createSandlotExtension(dependencies) {
    return function sandlot(pi) {
        const session = {
            dependencyWarnings: [],
        ...
        const childEnvironment = () => {
            ...
            return (dependencies.buildChildEnvironment ?? buildChildEnvironment)(dependencies.hostEnvironment, snapshot.policy.environment, session.environment);
        };
        ...
        pi.on("session_start", async (_event, ctx) => {
            ...
            if (dependencies.processState.poisonedError !== undefined) {
                ...
                if (ctx.hasUI)
                    ctx.ui.setStatus("sandlot", "⚠ Sandlot poisoned");
                reportDiagnostic(ctx, `${diagnosticError(dependencies.processState.poisonedError)} Restart Pi before using protected operations.`, "error");
                return;
            }
            ...
            try {
                ...
                if (!policy.enabled) {
                    ...
                    if (ctx.hasUI)
                        ctx.ui.setStatus("sandlot", "🔓 Sandlot disabled");
                    reportDiagnostic(ctx, "Sandlot is disabled by trusted user policy; protected operations will run locally.", "warning");
                    return;
                }
                ...
                const config = withTrustedWorkerPaths(dependencies.toSandboxRuntimeConfig(policy), workerPaths.trustedReadPaths, workerPaths.immutablePaths, workerPaths.rgPath, workerPaths.seccompApplyPath, workerPaths.bwrapPath, workerPaths.socatPath);
                ...
                if (ctx.hasUI) {
                    for (const warning of session.dependencyWarnings)
                        ctx.ui.notify(redactDiagnosticText(warning), "warning");
                }
            }
            catch (error) {
                dependencies.imageProcessor.clear();
                ...
                if (ctx.hasUI)
                    ctx.ui.setStatus("sandlot", "⚠ Sandlot failed");
        ...
        pi.on("user_bash", () => {
            const state = dependencies.runtime.snapshot().state;
            if (state === "ready" || state === "disabled-by-user")
                return { operations: userBashOperations };
            return {
                result: {
...
function policyExecutablePaths(policy) {
    return unique([
...
function withTrustedWorkerPaths(config, trustedReadPaths, immutablePaths, rgPath, seccompApplyPath, bwrapPath, socatPath) {
    const staged = {
        ...config,
...
function securityExecutablePaths(config) {
    return [
...
function productionProcessState() {
...
function sessionEnvironment(ctx) {
    return {
...
function setStatus(ctx, value) {
    if (ctx.hasUI)
        ctx.ui.setStatus("sandlot", value);
}
function clearStatus(ctx) {
    if (ctx.hasUI)
        ctx.ui.setStatus("sandlot", undefined);
}
...
function reportDiagnostic(ctx, message, type) {
    if (ctx.hasUI)
