import { DEFAULT_MAX_BYTES, formatSize, createEditTool, createEditToolDefinition, createLsTool, createLsToolDefinition, createReadTool, createReadToolDefinition, createWriteTool, createWriteToolDefinition, getPackageDir, truncateHead, } from "@earendil-works/pi-coding-agent";
 ...
 */
export function createFileOperations(client, context) {
    return {
        read: {
...
export function createSandlotReadTool(dependencies) {
    ...
    return {
        ...
        async execute(toolCallId, input, signal, onUpdate, context) {
            ...
            if (snapshot.state === "disabled-by-user") {
                return executeWithPiContext(createLocalReadTool(context.cwd, dependencies.createLocalReadOperations), toolCallId, input, signal, onUpdate, context);
            }
            throwIfAborted(signal);
            try {
                const result = await executeReadyReadWithAbort(input, context.cwd, signal, createFileOperations(dependencies.client, captureWorkerContext(snapshot, toolCallId, signal)).read, context, dependencies.processImage ?? processImageWithPinnedPi);
...
export function createSandlotWriteTool(dependencies) {
...
export function createSandlotEditTool(dependencies) {
...
export function createSandlotLsTool(dependencies) {
...
function createReadyWriteTool(cwd, client, context) {
...
export class PinnedPiImageProcessor {
    ...
    #moduleUrl;
    #active = new Set();
    constructor(createWorker = createImageProcessorWorker) {
        this.createWorker = createWorker;
    }
    bind(imageProcessorPath) {
        if (!isAbsolute(imageProcessorPath))
            throw new Error("Validated Pi image processor path must be absolute");
        const moduleUrl = pathToFileURL(imageProcessorPath).href;
        if (this.#moduleUrl === moduleUrl)
            return;
    ...
    process = async (bytes, mimeType, options = {}) => {
    ...
    async abortAll() {
        ...
        const failure = terminations.find((result) => result.status === "rejected");
        if (failure !== undefined)
            throw failure.reason;
    }
    startExecution(worker, signal) {
        let settled = false;
        let aborted = false;
        let received;
        ...
        const onSignalAbort = () => { void abort().catch(() => undefined); };
 ...
 */
async function executeReadyRead(input, cwd, signal, operations, context, processImage) {
    throwIfAborted(signal);
    const absolutePath = resolveReadPathLexically(input.path, cwd);
    await operations.access(absolutePath);
    throwIfAborted(signal);
    const atomicRead = await operations
    ...
    if (mimeType) {
        ...
        if (!processed.ok || processed.data === undefined || processed.mimeType === undefined || processed.hints === undefined) {
            ...
            if (nonVisionImageNote !== undefined)
                text += `\n${nonVisionImageNote}`;
...
async function executeReadyReadWithAbort(input, cwd, signal, operations, context, processImage) {
    throwIfAborted(signal);
    return executeReadyRead(input, cwd, signal, operations, context, processImage);
}
function resolveReadPathLexically(input, cwd) {
...
function getNonVisionImageNote(context) {
    if (typeof context !== "object" || context === null || !("model" in context))
        return undefined;
    const model = context.model;
    if (typeof model !== "object" || model === null || !("input" in model) || !Array.isArray(model.input))
        return undefined;
...
async function processImageWithPinnedPi(bytes, mimeType) {
...
export function resolvePinnedPiImagePaths(piPackageDirectory) {
    let piPackageRoot;
    try {
    ...
    assertPackageMetadata(join(piPackageRoot, "package.json"), "@earendil-works/pi-coding-agent", "0.84.2", "Pinned host Pi package");
    const imageModules = [
        "image-process.js",
        "image-convert.js",
        "image-resize.js",
    ...
    const photonPackageRoot = join(piPackageRoot, "node_modules", "@silvia-odwyer", "photon-node");
    assertPackageMetadata(join(photonPackageRoot, "package.json"), "@silvia-odwyer/photon-node", "0.3.4", "Pinned Photon package");
    const photonEntryPath = requiredHostGraphFile(join(photonPackageRoot, "photon_rs.js"), "Pinned Photon image module");
    const photonWasmPath = requiredHostGraphFile(join(photonPackageRoot, "photon_rs_bg.wasm"), "Pinned Photon WASM");
...
function assertPackageMetadata(packageJsonPath, expectedName, expectedVersion, label) {
    const canonicalPath = requiredHostGraphFile(packageJsonPath, `${label} metadata`);
    let metadata;
    try {
        metadata = JSON.parse(readFileSync(canonicalPath, "utf8"));
    }
    catch (error) {
        throw pinnedImageGraphError(`${label} metadata`, error);
    }
    if (metadata.name !== expectedName || metadata.version !== expectedVersion) {
        throw new Error(`${label} must be ${expectedName}@${expectedVersion}; reinstall the exact pinned Pi 0.84.2 host package`);
    }
}
function requiredHostGraphFile(path, label) {
    let metadata;
    try {
...
function pinnedImageGraphError(label, cause) {
    return new Error(`${label} is unavailable in the pinned host image graph; reinstall @earendil-works/pi-coding-agent@0.84.2 and restart Pi`, { cause });
}
function createReadyEditTool(cwd, client, context) {
    return createEditTool(cwd, { operations: createFileOperations(client, context).edit });
}
function createReadyLsTool(cwd, client, context) {
    return createLsTool(cwd, { operations: createFileOperations(client, context).ls });
}
function createLocalReadTool(cwd, createOperations) {
    return createOperations === undefined ? createReadTool(cwd) : createReadTool(cwd, { operations: createOperations() });
}
function createLocalWriteTool(cwd, createOperations) {
    return createOperations === undefined ? createWriteTool(cwd) : createWriteTool(cwd, { operations: createOperations() });
}
function createLocalEditTool(cwd, createOperations) {
    return createOperations === undefined ? createEditTool(cwd) : createEditTool(cwd, { operations: createOperations() });
}
function createLocalLsTool(cwd, createOperations) {
...
function executeWithPiContext(tool, toolCallId, input, signal, onUpdate, context) {
