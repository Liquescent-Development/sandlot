...
The manual release workflow accepts only a version merged into `main`. Its read-only macOS job validates the version sources, runs the complete `npm run release:verify` gate, proves the generated tree is clean, and builds the tarball and checksum. A separate job receives only that verified handoff, rechecks remote state with `contents: write`, creates and fills a draft, publishes it, and requires GitHub to report the resulting release as immutable. The write-authorized job neither checks out nor executes repository code.
...
A publication failure may leave a draft. The workflow reports its release ID and tag, deliberately leaves it for inspection, and will not delete or overwrite it. Before any retry, inspect the failed run and all matching remote state:
...
A missing tag returns `404`. If a draft exists, compare its recorded ID, target, notes, and assets with the verified run. After confirming `draft: true` and confirming that the tag is absent, a maintainer may deliberately remove only that recorded draft before rerunning:

```bash
gh api --method DELETE -H "X-GitHub-Api-Version: 2026-03-10" repos/Liquescent-Development/sandlot/releases/RELEASE_ID
```

Never delete a published release or tag, and never retry blindly when any tag or published release exists. If publication succeeded but the final immutable assertion failed, stop and investigate the repository setting and returned release state rather than modifying remote state.
...
```bash
gh api -H "X-GitHub-Api-Version: 2026-03-10" repos/Liquescent-Development/sandlot/releases/tags/v0.1.0 --jq '{tag_name,target_commitish,draft,prerelease,immutable,assets:[.assets[]|{name,size,digest}]}'
...
The release notes must exactly match the curated `CHANGELOG.md` section for that version. The asset names, sizes, and GitHub `sha256:` digests must agree with the downloaded files.
