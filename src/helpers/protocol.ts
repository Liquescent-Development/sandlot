...
export type FileWorkerRequest =
  | { version: 1; operation: "read"; path: string }
  | { version: 1; operation: "readImage"; path: string }
  | { version: 1; operation: "access"; path: string; mode: "read" | "write" }
  | { version: 1; operation: "write"; path: string; content: string; createParents: boolean }
  | { version: 1; operation: "mkdir"; path: string; recursive: boolean }
  | { version: 1; operation: "stat"; path: string }
  | { version: 1; operation: "readdir"; path: string }
  | { version: 1; operation: "mime"; path: string };

export type SearchWorkerRequest =
  | { version: 1; operation: "find"; cwd: string; pattern: string; ignore: string[]; limit: number }
  | {
    version: 1;
    operation: "grep";
    cwd: string;
...
export function decodeRequest(encoded: string): WorkerRequest {
  if (Buffer.byteLength(encoded) > MAX_WORKER_REQUEST_BYTES) {
    throw new WorkerProtocolError(`worker request exceeds ${MAX_WORKER_REQUEST_BYTES}-byte limit`);
  }
...
export function decodeResponse<T = unknown>(encoded: string): WorkerResponse<T> {
  ...
  if (typeof record.ok !== "boolean") throw new WorkerProtocolError("worker response ok must be a boolean");
...
function validateRequest(value: unknown): asserts value is WorkerRequest {
...
function parseJson(encoded: string, kind: "request" | "response"): unknown {
  try {
...
function requireExactKeys(record: Record<string, unknown>, expected: readonly string[], context: string): void {
  const expectedSet = new Set(expected);
  const unexpected = Object.keys(record).find((key) => !expectedSet.has(key));
  if (unexpected !== undefined) throw new WorkerProtocolError(`${context} has unexpected field: ${unexpected}`);
  const missing = expected.find((key) => !Object.hasOwn(record, key));
  if (missing !== undefined) throw new WorkerProtocolError(`${context} is missing field: ${missing}`);
}

function requireString(value: unknown, context: string): asserts value is string {
  if (typeof value !== "string") throw new WorkerProtocolError(`${context} must be a string`);
}

function requireBoolean(value: unknown, context: string): asserts value is boolean {
  if (typeof value !== "boolean") throw new WorkerProtocolError(`${context} must be a boolean`);
}

function requireStringArray(value: unknown, context: string): asserts value is string[] {
  if (!Array.isArray(value) || !value.every((entry) => typeof entry === "string")) {
    throw new WorkerProtocolError(`${context} must be an array of strings`);
  }
}

function requireSafeLimit(value: unknown, context: string): asserts value is number {
