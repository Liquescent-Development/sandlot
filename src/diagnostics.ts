...
export function renderDiagnosticSnapshot(input: DiagnosticSnapshotInput): string {
  ...
  if (policy !== undefined) {
    lines.push(
      `policy: enabled=${String(policy.enabled)}, network mode=${policy.networkMode}, allowed domains=${policy.network.allowedDomains.length}, denied reads=${policy.filesystem.denyRead.length}, allowed writes=${policy.filesystem.allowWrite.length}, trusted custom tools=${policy.trustedCustomTools.length}`,
    );
  }
  if (input.runtime.error !== undefined) lines.push(`error: ${redactDiagnosticText(input.runtime.error)}`);
  lines.push(`dependency warnings: ${input.dependencyWarnings.length}`);
  for (const warning of input.dependencyWarnings) lines.push(`- ${redactDiagnosticText(warning)}`);
  ...
  for (const violation of formattedViolations) lines.push(`- ${redactDiagnosticText(violation)}`);
  return lines.join("\n");
}
...
function ownershipStatus(tools: readonly ToolInfo[], sourcePath: string): "ok" | "failed" {
  try {
